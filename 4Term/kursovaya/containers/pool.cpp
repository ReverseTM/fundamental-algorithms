...
pool::pool(memory * allocator): _pool(new splay_tree<std::string, scheme, string_comparer>()), _allocator(allocator)
{
...
pool::pool(const pool &other)
    : _pool(new splay_tree<std::string, scheme, string_comparer>(*reinterpret_cast<splay_tree<std::string, scheme, string_comparer>*>(other._pool))), _allocator(other._allocator)
{
